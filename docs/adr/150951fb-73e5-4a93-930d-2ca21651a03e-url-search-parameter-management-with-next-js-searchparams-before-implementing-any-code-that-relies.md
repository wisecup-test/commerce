...
1. MUST: Before implementing any code that relies on a versioned dependency, the consumer MUST locate the project's dependency manifest, identify the build tool, inspect the repository's lock or resolution artifact to determine the exact resolved version, and consult the official documentation for that specific version to confirm API existence and behavior.

## Policy Block

- MUST Before implementing any code that relies on a versioned dependency, the consumer MUST locate the project's dependency manifest, identify the build tool, inspect the repository's lock or resolution artifact to determine the exact resolved version, and consult the official documentation for that specific version to confirm API existence and behavior.
