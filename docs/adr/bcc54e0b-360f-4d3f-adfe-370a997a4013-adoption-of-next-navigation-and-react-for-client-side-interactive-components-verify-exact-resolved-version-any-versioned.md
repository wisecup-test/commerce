...
1. MUST: Verify the exact resolved version of any versioned dependency by inspecting the project's lock file before implementing code that relies on its APIs.

## Policy Block

- MUST Verify the exact resolved version of any versioned dependency by inspecting the project's lock file before implementing code that relies on its APIs.
