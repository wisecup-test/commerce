...
1. MUST: Use `next/navigation` for interacting with URL query parameters within client-side components, specifically for reading values like `searchParams.get()`.

## Policy Block

- MUST Use `next/navigation` for interacting with URL query parameters within client-side components, specifically for reading values like `searchParams.get()`.
