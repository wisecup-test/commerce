...
1. MUST: Webhook validation logic that compares secrets MUST source the expected secret value from the same configuration mechanism used for API credentials

## Policy Block

- MUST Webhook validation logic that compares secrets MUST source the expected secret value from the same configuration mechanism used for API credentials
