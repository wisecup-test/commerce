...
1. MUST: Cart identifier reads MUST use the async cookie retrieval API to obtain session state before performing cart operations.

## Policy Block

- MUST Cart identifier reads MUST use the async cookie retrieval API to obtain session state before performing cart operations.
