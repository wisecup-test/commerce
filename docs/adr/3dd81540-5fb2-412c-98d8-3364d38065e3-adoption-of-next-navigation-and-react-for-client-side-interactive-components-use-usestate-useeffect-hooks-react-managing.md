...
1. MUST: Use `useState` and `useEffect` hooks from `react` for managing component-local state and side effects in client-side components.

## Policy Block

- MUST Use `useState` and `useEffect` hooks from `react` for managing component-local state and side effects in client-side components.
