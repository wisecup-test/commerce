...
1. MAY: Direct manipulation of the browser's `window.location.search` MAY be used only for scenarios where `next/navigation` APIs are insufficient or introduce undesirable side effects.

## Policy Block

- MAY Direct manipulation of the browser's `window.location.search` MAY be used only for scenarios where `next/navigation` APIs are insufficient or introduce undesirable side effects.
