...
1. SHOULD: SHOULD ensure that Promises consumed by the `use` hook are stable and do not change unnecessarily to prevent excessive re-renders or re-suspensions.

## Policy Block

- SHOULD SHOULD ensure that Promises consumed by the `use` hook are stable and do not change unnecessarily to prevent excessive re-renders or re-suspensions.
