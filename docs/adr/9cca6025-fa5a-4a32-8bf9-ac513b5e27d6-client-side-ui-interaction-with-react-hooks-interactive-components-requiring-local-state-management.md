...
1. MUST: Interactive UI components requiring local state management MUST utilize the `useState` hook from `react`.

## Policy Block

- MUST Interactive UI components requiring local state management MUST utilize the `useState` hook from `react`.
