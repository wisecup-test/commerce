# Client-Side UI Interaction with React Hooks: Interactive Components Requiring Side Effects Data
...
1. MUST: Interactive UI components requiring side effects (e.g., data fetching, subscriptions, DOM manipulation) MUST utilize the `useEffect` hook from `react`.

## Policy Block

- MUST Interactive UI components requiring side effects (e.g., data fetching, subscriptions, DOM manipulation) MUST utilize the `useEffect` hook from `react`.
