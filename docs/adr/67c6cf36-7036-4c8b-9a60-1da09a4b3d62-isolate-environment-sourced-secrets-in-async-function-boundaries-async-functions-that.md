...
1. SHOULD: Async functions that use secrets for external authentication SHOULD fail fast with non-revealing error messages when secrets are missing or invalid.

## Policy Block

- SHOULD Async functions that use secrets for external authentication SHOULD fail fast with non-revealing error messages when secrets are missing or invalid.
