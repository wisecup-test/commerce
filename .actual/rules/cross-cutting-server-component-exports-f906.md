# Adopt Async Function Exports as Standard Concurrency Model for Server Components and Actions: Server Component Exports

These rules are ALWAYS ACTIVE for all server-side component rendering, data fetching, and server action implementations.

### Rules

- **R-ASYNC-001** MUST: All server component exports that perform data fetching, external API calls, or file system operations MUST be declared as async functions.
...
# (Exact command depends on project's build tool and test runner — inspect package.json or build config)
...
# (Exact command depends on project's type checker — typically `tsc` or equivalent)
...
# (Exact command depends on project's linter — typically `eslint` or equivalent)
```
