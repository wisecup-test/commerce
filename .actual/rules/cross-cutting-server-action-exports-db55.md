...
- **R-ASYNC-001** MUST: All server action exports that mutate state, interact with cache layers, or call external services MUST be declared as async functions.
...
# (Exact command depends on project's type checker — derive from tsconfig.json or similar configuration)
...
# (Exact command depends on project's linter — derive from .eslintrc or similar configuration)
```
